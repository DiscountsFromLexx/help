...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('.history-btn')?.addEventListener('click', () => {
        ...
        // Спочатку перевіряємо, чи є вже відкрита модалка
        let modal = document.getElementById('help-modal');
    
        if (!modal) {
            // Якщо модалки немає — створюємо нову
            ...
            contentContainer.style.height = isTelegramMiniApp ? 'calc(100vh - 60px)' : '90vh';
            contentContainer.style.borderRadius = '16px';
            contentContainer.style.overflow = 'hidden';
            contentContainer.style.boxShadow = '0 10px 40px rgba(0,0,0,0.6)';
        ...
        // Очищаємо попередній вміст і вставляємо новий iframe
        const contentContainer = modal.querySelector('div');
        contentContainer.innerHTML = ''; // очищаємо старе
